...
class Auth extends My_Controller {
	...
	public function register(){
		...
		$this->load->view('admin/templates/header_ui', $this->data);
		$this->load->view('admin/auth/register', $this->data);
		$this->load->view('admin/templates/footer_ui', $this->data);
	}
	...
	public function login() {
		...
		if ( $_POST ){
			...
			if($email && $password){
				...
				$condition = 'lower(email) = "'.strtolower($email).'" AND password = "'.$password.'" AND is_active = 1 AND group_id = 1';
//				$user = $this->common->get('users', $condition);
		...
		if($_POST) {
			...
			if( $email ) {
				...
				if( !empty($user) ) {
					...
					$click_here = '<a href="'.site_url('admin/reset_password/'.$key).'" target="_blank">'.site_url('admin/reset_password/'.$key).' </a>';
					//send reset mail
					$params = array(
	...
	public function set_password( $key = "" ){
		...
		$this->load->view('admin/templates/header_ui', $this->data);
		$this->load->view('admin/auth/set_password', $this->data);
		$this->load->view('admin/templates/footer_ui', $this->data);
	}
